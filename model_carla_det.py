...
class CarlaDetModel(nn.Module):
    ...
    def prepare_common_tensors(self, feed):
        ...
        if self.set_name=='test' or self.set_name=='val' or self.set_name=='train':
            print('fixing centroid')
            # fixed centroid
        ...
        self.scorelist_s = __u(utils.misc.rescore_lrtlist_with_inbound(
            __p(self.lrtlist_camRs), __p(self.tidlist_s), self.Z, self.Y, self.X, self.vox_util, pad=2.0))
        ...
        for b in list(range(self.B)):
            if torch.sum(self.scorelist_s[:,0]) < (self.B/2): # not worth it; return early
                ...
                return False # not OK; do not try to train on this
    ...
    def run_train(self, feed):
        ...
        if hyp.do_feat3d:
            ...
            feat_halfmemR0 = self.vox_util.apply_4x4_to_vox(self.camRs_T_camXs[:,0], feat_halfmemX0)
            valid_halfmemR0 = self.vox_util.apply_4x4_to_vox(self.camRs_T_camXs[:,0], valid_halfmemX0)
            ...
            self.summ_writer.summ_feat('feat3d/feat_halfmemR0', feat_halfmemR0, pca=True)
        ...
        if hyp.do_det:
            ...
            # print('lenlist_halfmem_e', utils.geom.get_lenlist_from_lrtlist(lrtlist_halfmemR_e))
            lrtlist_camX_e = self.vox_util.apply_ref_T_mem_to_lrtlist(lrtlist_halfmemX_e, self.Z2, self.Y2, self.X2)
            # print('lenlist_cam_e', utils.geom.get_lenlist_from_lrtlist(lrtlist_camR_e))
            ...
            if Ne > 0 and Ng > 0:
                ...
                self.summ_writer.summ_lrtlist_bev('det/boxlist_e', occ_memX0[:1],
                                                  lrtlist_e,
                                                  scorelist_e,
                                                  tidlist_e,
                                                  self.vox_util, 
                ...
                self.summ_writer.summ_lrtlist_bev('det/boxlist', occ_memX0[0:1],
                                                  torch.cat((lrtlist_e, lrtlist_g), dim=1),
                                                  torch.cat((ious_e, ious_e.new_ones(1, Ng)), dim=1),
                                                  torch.cat((ious_e.new_ones(1, Ne).long(),
                                                             ious_e.new_ones(1, Ng).long() * 2), dim=1),
            ...
            for ind, overlap in enumerate(ious):
                self.summ_writer.summ_scalar('ap/%.2f_iou' % overlap, maps_3d[ind])

            
            # axboxlist_camRs = __u(utils.geom.inflate_to_axis_aligned_boxlist(__p(self.boxlist_camRs)))
            # axlrtlist_camRs = __u(utils.geom.convert_boxlist_to_lrtlist(__p(axboxlist_camRs)))
            # if self.include_image_summs:
            ...
            # boxlist_memR = utils.vox.convert_boxlist_camR_to_memR(self.boxlist_camRs[:,0], self.Z2, self.Y2, self.X2)
            # axboxlist_memR = utils.geom.inflate_to_axis_aligned_boxlist(boxlist_memR)

            # featRs = utils.vox.apply_4x4s_to_voxs(self.camRs_T_camXs, feat_memXs)
            ...
            # boxlist_camR_e = utils.vox.convert_boxlist_memR_to_camR(boxlist_memR_e, self.Z2, self.Y2, self.X2)
            # lrtlist_camR_e = utils.geom.convert_boxlist_to_lrtlist(boxlist_camR_e)
            # if self.include_image_summs:
            ...
            # boxlist_e, boxlist_g, scorelist_e, scorelist_g = utils.eval.drop_invalid_boxes(
            #     boxlist_e, boxlist_g, scorelist_e, scorelist_g)

            # ious = [0.3, 0.4, 0.5, 0.6, 0.7]
            # maps = utils.eval.get_mAP(boxlist_e, scorelist_e, boxlist_g, ious)
            # for ind, overlap in enumerate(ious):
